#!/usr/bin/env bash
# Install the tested Nemotron runtime and OpenCode without administrator access.
set -euo pipefail
umask 077
...
if [[ ! -x $root/envs/nemotron-vllm/bin/python ]]; then
  "$root/bootstrap/bin/uv" venv --python python3 "$root/envs/nemotron-vllm"
fi
"$root/bootstrap/bin/uv" pip sync --python "$root/envs/nemotron-vllm/bin/python" \
  "$repo/env/nemotron-requirements.lock"
"$root/bootstrap/bin/uv" pip check --python "$root/envs/nemotron-vllm/bin/python"
...
echo "Setup complete. Runtime: $root/envs/nemotron-vllm"
