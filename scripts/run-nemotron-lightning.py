#!/usr/bin/env python3
"""Own one server and optional client command inside a bounded Slurm job for Nemotron Lightning.

Usage: python scripts/run-nemotron-lightning.py [-- command argument ...]
...
def main():
    ...
    if command[:1] == ['--']:
        command = command[1:]
    port_text = os.environ.get('NEMOTRON_PORT', '8000')
    if not port_text.isascii() or not port_text.isdecimal() or not 1024 <= int(port_text) <= 65535:
        raise ValueError('NEMOTRON_PORT must be in 1024..65535')
    port = int(port_text)
    repo = Path(__file__).resolve().parent.parent
    stamp = datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    directory = repo / '.local' / f'nemotron-lightning-{stamp}-{os.getpid()}'
    ...
    try:
        with (directory / 'server.log').open('w') as log:
            server = subprocess.Popen(['bash', str(repo / 'scripts/serve-nemotron-lightning.sh')],
                ...
                try:
                    ...
                    if health_ok:
                        ...
                        with opener.open(req_models, timeout=2) as response:
                            ...
                            if 'nemotron-3.5-lightning' in model_ids and server.poll() is None:
                                break
                except (OSError, ValueError, urllib.error.URLError):
                    pass
                time.sleep(1)
            print(f'Server ready in {time.monotonic() - start:.2f}s; pid={server.pid}', flush=True)
            os.environ['NEMOTRON_SERVER_LOG'] = log.name
            os.environ['NEMOTRON_SERVER_PID'] = str(server.pid)
            os.environ['NEMOTRON_STARTUP_SECONDS'] = str(time.monotonic() - start)
            if command:
...
if __name__ == '__main__':
    try:
        ...
        sys.exit(f'Nemotron Lightning: {exc}')
